...
import { selectCurrentProject, selectCurrentVersion, selectRunningTasks } from '@renderer/store/imageStudio'
import { Button, Empty, Progress, Spin, Tooltip } from 'antd'
import { Download, Image as ImageIcon, Maximize2, RotateCcw } from 'lucide-react'
...
const StudioCanvas: FC = () => {
  ...
  const activeImage = selectedImage
    ? outputImages.find((img) => img.key === selectedImage)
    : outputImages[0]
  ...
  if (!currentProject) {
    return (
      <CanvasContainer>
        <EmptyState>
          <Empty
            image={<EmptyIcon><ImageIcon size={64} strokeWidth={1} /></EmptyIcon>}
            description={
              <EmptyDescription>
                <span>{t('image_studio.canvas.empty')}</span>
                <EmptyHint>{t('image_studio.canvas.hint')}</EmptyHint>
              </EmptyDescription>
            }
          />
        </EmptyState>
    ...
    <CanvasContainer>
      <EmptyState>
        <Empty
          ...
          description={
            <EmptyDescription>
              <span>{t('image_studio.canvas.no_output')}</span>
